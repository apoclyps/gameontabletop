...
def upgrade() -> None:
    ...
    op.create_index(
        op.f("ix_guest_tokens_token"), "guest_tokens", ["token"], unique=True
    )
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(
            ["guest_token_id"], ["guest_tokens.id"], ondelete="CASCADE"
        ),
        sa.PrimaryKeyConstraint("id"),
    )
...
def downgrade() -> None:
    ...
    op.drop_constraint(
        "fk_polls_chosen_option", "availability_polls", type_="foreignkey"
    )
