...
def no_email():
    with patch("app.api.auth.send_verification_email"), patch(
        "app.api.auth.send_password_reset_email"
    ):
        yield


async def register(client: AsyncClient, email="user@example.com", username="testuser", password="Password1"):
    return await client.post("/api/auth/register", json={"email": email, "username": username, "password": password})
...
async def register_and_verify(client: AsyncClient, email="user@example.com", username="testuser"):
...
class TestLogin:
    async def test_success(self, client: AsyncClient):
        ...
        r = await client.post("/api/auth/login", json={"email": "user@example.com", "password": "Password1"})
        assert r.status_code == 200
        data = r.json()
    ...
    async def test_wrong_password(self, client: AsyncClient):
        await register_and_verify(client)
        r = await client.post("/api/auth/login", json={"email": "user@example.com", "password": "wrongpass"})
    ...
    async def test_unknown_email(self, client: AsyncClient):
        r = await client.post("/api/auth/login", json={"email": "no@example.com", "password": "Password1"})
        assert r.status_code == 401
...
class TestRefresh:
    async def test_success(self, client: AsyncClient):
        await register_and_verify(client)
        login = await client.post("/api/auth/login", json={"email": "user@example.com", "password": "Password1"})
...
class TestForgotPassword:
    async def test_known_email_always_200(self, client: AsyncClient):
        await register_and_verify(client)
        r = await client.post("/api/auth/forgot-password", json={"email": "user@example.com"})
        assert r.status_code == 200
...
class TestResetPassword:
    async def test_success(self, client: AsyncClient):
        ...
        r = await client.post("/api/auth/reset-password", json={"token": token, "new_password": "NewPass123"})
        assert r.status_code == 200
        # Login with new password
        r2 = await client.post("/api/auth/login", json={"email": "user@example.com", "password": "NewPass123"})
        assert r2.status_code == 200

    async def test_invalid_token(self, client: AsyncClient):
        r = await client.post("/api/auth/reset-password", json={"token": "bad", "new_password": "NewPass123"})
        assert r.status_code == 400
